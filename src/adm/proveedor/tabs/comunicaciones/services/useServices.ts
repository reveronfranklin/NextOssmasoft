...
const useServices = () => {
    ...
    const getList = useCallback(async (filters: SisBancoFilterDto): Promise<any> => {
        try {
            setLoading(true)
            const response = await ossmmasofApi.post<IResponse<SisBancoResponseDto>>(UrlServices.GET_COMUNICACIONES, filters)
    ...
    const store = useCallback(async (payload: SisBancoCreateDto): Promise<any> => {
        try {
            setLoading(true)
            const response  = await ossmmasofApi.post<IResponse<SisBancoResponseDto>>(UrlServices.CREATE_COMUNICACIONES, payload)
            const message   = 'Comunicación creada exitosamente'
    ...
    const update = useCallback(async (payload: SisBancoUpdateDto): Promise<any> => {
        try {
            setLoading(true)
            const response  = await ossmmasofApi.post<IResponse<SisBancoResponseDto>>(UrlServices.UPDATE_COMUNICACIONES, payload)
            const message   = 'Comunicación actualizada exitosamente'
    ...
    const remove = useCallback(async (payload: SisBancoDeleteDto): Promise<any> => {
        try {
            setLoading(true)
            const response  = await ossmmasofApi.post<IResponse<SisBancoDeleteDto>>(UrlServices.DELETE_COMUNICACIONES, payload)
            const message   = 'Comunicación eliminada exitosamente'
