...
import {  setPreDetalleDocumentoBloqueado,setVerPreDetalleDocumentoBloqueadoActive } from 'src/store/apps/presupuesto';
...
import TableBasicBloqueado from './TableBasicBloqueado'
...
const DialogPreVSaldoDocumentosPagadoInfo = () => {

  // ** States
  const dispatch = useDispatch();
  const {verPreDetalleDocumentoBloqueadoActive,preVSaldoSeleccionado} = useSelector((state: RootState) => state.presupuesto)
  ...
  const fetchTableData = useCallback(
    async (codigoPresupuesto:number,codigoSaldo:number) => {
      ...
      const responseAll= await ossmmasofApi.post<any>('/PreVDocBloqueado/GetAllByCodigoSaldo',filter);
      let data:any=[];
      if(responseAll.data.data!=null){
        data=responseAll.data.data
      }
      dispatch(setPreDetalleDocumentoBloqueado(data));
      props.data=data
...
{/*       <CardContent sx={{ textAlign: 'center', '& svg': { mb: 2 } }}>
      ...
      <Dialog
        fullWidth
        open={verPreDetalleDocumentoBloqueadoActive}
        maxWidth='lg'
        scroll='body'
...
export default DialogPreVSaldoDocumentosPagadoInfo
