import { Autocomplete, Box, Card, CardActions, CardHeader, Grid, TextField } from '@mui/material'
...
const ConteoDetalleCompareList = () => {
  ...
  const { listBmConteoResponseDto, listBmConteoDetalleResponseDto } = useSelector((state: RootState) => state.bmConteo)
  ...
  const handleConteo = (e: any, value: any) => {
    console.log('handler conteo seleccionado', value)
    if (value != null) {
  ...
  const refreshData = async (conteo: IBmConteoResponseDto) => {
    dispatch(setListBmConteoDetalleResponseDto([]))
    if (conteo && conteo.codigoBmConteo > 0) {
      setLoading(true)
