...
const urlProduction = process.env.NEXT_PUBLIC_BASE_URL_API_GATEWAY_NEST_PRODUCTION
const urlDevelopment = process.env.NEXT_PUBLIC_BASE_URL_API_GATEWAY_NEST

export const ossmmasofApiGateway = axios.create({
    ...
    if (originalConfig && originalConfig.url !== '/login' && err.response) {
      ...
      if (err.response.status === 401 && !originalConfig._retry) {
        ...
        try {
          ...
          return ossmmasofApiGateway(originalConfig)
        } catch (_error) {
