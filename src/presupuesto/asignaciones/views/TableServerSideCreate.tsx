...
const TableServerSideCreate = () => {
 ...
 const {listPreAsignacionesCreate} = useSelector((state: RootState) => state.preAsignaciones)
    ...
    async (filter:IPreAsignacionesFilterDto) => {
      ...
      if(responseAll.data.data){
       ...
       // setLinkData(responseAll.data.linkData);
        dispatch(setListPreAsignacionesCreate(responseAll.data.data));
  ...
  useEffect(() => {
    if(listpresupuestoDtoSeleccionado.codigoPresupuesto>0 && preMtrUnidadEjecutoraSeleccionado.codigoIcp>0){
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [listpresupuestoDtoSeleccionado,preMtrUnidadEjecutoraSeleccionado])
    ...
    <Card>
     ...
     { loading  ? (
       <Spinner sx={{ height: '100%' }} />
        ...
        rows={listPreAsignacionesCreate}
        rowCount={total}
        columns={columns}
        pageSize={pageSize}
        sortingMode='server'
...
export default TableServerSideCreate
